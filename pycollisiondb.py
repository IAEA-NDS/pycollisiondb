...
class PyCollision:
    ...
    def make_query(self, query_data):
        ...
        if r.status_code != 200:
            raise PyCollisionDBConnectionError(f'Could not retrieve data: HTTP'
    ...
    def plot_datasets(self, ax, pks=None, reaction_texts=None, **kwargs):
        if pks is None:
            pks = [pk for reaction_text in reaction_texts for pk in self.pks[reaction_text]]
    ...
    def _get_plot_metadata(self, pks):
        ...
        for pk in pks[1:]:
            if self.datasets[pk].metadata['data_type'] != data_type:
                raise PyCollisionDBPlotError('Data types not all the same in requested plot.')
